...
operation_gateway_service = OperationsGatewayServiceStub(channel)
...
print('OpenDebit card account response:', open_debit_card_account_response)

make_top_up_request = MakeTopUpOperationRequest(status=OperationStatus.OPERATION_STATUS_COMPLETED,
                                                amount=fake.amount(),
                                                card_id=open_debit_card_account_response.account.cards[0].id,
                                                account_id=open_debit_card_account_response.account.id)
make_top_up_operation_response: MakeTopUpOperationResponse = operation_gateway_service.MakeTopUpOperation(make_top_up_request)
...
get_operation_receipt_response: GetOperationReceiptResponse = operation_gateway_service.GetOperationReceipt(get_operation_receipt_request)
